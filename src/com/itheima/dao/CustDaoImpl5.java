...
public class CustDaoImpl5 {
	public int addCust(Cust cust) {
		String sql = "insert into moper values (null,?,?,?,?,?,?,?,?)";
		try{
			Cust cunzai = findUserByProductid(cust.getProductid());
			if(cunzai!=null){
				updateCust(cust);
				if(cunzai.getParame1p().equals("ºÏ¸ñ")){
					return 0;
				}
				return 1;
			}else{
			...
			System.out.println(cust.getProductid()+"addMOP");
			return 0;
			}
		}catch (Exception e) {
			e.printStackTrace();
	...
	public Cust findUserByProductid(String productid) {
		String sql = "select * from moper where productid = ?";
		try{
			QueryRunner runner = new QueryRunner(DaoUtils.getSource());
	...
	public Cust findUserById(String id) {
		String sql = "select * from moper where id = ?";
		try{
			QueryRunner runner = new QueryRunner(DaoUtils.getSource());
	...
	public void updateCust(Cust cust) {
		String sql = "update moper set xinghao=?,pihao=?,parame1=?,parame1p=?,parame2=?,parame2p=?,time=? where productid=?";
		try{
			QueryRunner runner = new QueryRunner(DaoUtils.getSource());
	...
	public void delCustByID(String id) {
		String sql = "delete from moper where id = ?";
		try{
			QueryRunner runner = new QueryRunner(DaoUtils.getSource());
	...
	public List<Cust> findCustByCond(Cust cust) {
		String sql = "select * from moper where 1=1 ";
	...
	public int getCountRow() {
		String sql = "select count(*) from moper";
		try{
			QueryRunner runner = new QueryRunner(DaoUtils.getSource());
	...
	public List<Cust> getCustByPage(int from, int count) {
		String sql = "select * from moper limit ?,?";
		try{
			QueryRunner runner = new QueryRunner(DaoUtils.getSource());
