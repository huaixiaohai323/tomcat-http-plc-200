...
public class CustDaoImpl4 {
	...
	public Cust findUserByProductid(String productid) {
		String sql = "select * from mopyi where productid = ?";
		try{
			QueryRunner runner = new QueryRunner(DaoUtils.getSource());
	...
	public Cust findUserById(String id) {
		String sql = "select * from mopyi where id = ?";
		try{
			QueryRunner runner = new QueryRunner(DaoUtils.getSource());
	...
	public void updateCust(Cust cust) {
		String sql = "update mopyi set xinghao=?,pihao=?,parame1=?,parame1p=?,parame2=?,parame2p=?,time=? where productid=?";
		try{
			QueryRunner runner = new QueryRunner(DaoUtils.getSource());
	...
	public void delCustByID(String id) {
		String sql = "delete from mopyi where id = ?";
		try{
			QueryRunner runner = new QueryRunner(DaoUtils.getSource());
	...
	public List<Cust> findCustByCond(Cust cust) {
		String sql = "select * from mopyi where 1=1 ";
	...
	public int getCountRow() {
		String sql = "select count(*) from mopyi";
		try{
			QueryRunner runner = new QueryRunner(DaoUtils.getSource());
	...
	public List<Cust> getCustByPage(int from, int count) {
		String sql = "select * from mopyi limit ?,?";
		try{
			QueryRunner runner = new QueryRunner(DaoUtils.getSource());
