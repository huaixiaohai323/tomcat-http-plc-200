...
public class CustDaoImpl7 {
	public void addCust(Cust cust) {
		String sql = "insert into shuijian values (null,?,?,?,?,?,?,?,?)";
		try{
	...
	public Cust findUserByProductid(String productid) {
		String sql = "select * from shuijian where productid = ?";
		try{
			QueryRunner runner = new QueryRunner(DaoUtils.getSource());
	...
	public List<Cust> getAllCust() {
		String sql = "select * from shuijian";
		try{
			QueryRunner runner = new QueryRunner(DaoUtils.getSource());
	...
	public Cust findUserById(String id) {
		String sql = "select * from shuijian where id = ?";
		try{
			QueryRunner runner = new QueryRunner(DaoUtils.getSource());
	...
	public void updateCust(Cust cust) {
		String sql = "update shuijian set xinghao=?,pihao=?,parame1=?,parame1p=?,parame2=?,parame2p=?,time=? where productid=?";
		try{
			QueryRunner runner = new QueryRunner(DaoUtils.getSource());
	...
	public void delCustByID(String id) {
		String sql = "delete from shuijian where id = ?";
		try{
			QueryRunner runner = new QueryRunner(DaoUtils.getSource());
	...
	public List<Cust> findCustByCond(Cust cust) {
		String sql = "select * from shuijian where 1=1 ";
	...
	public int getCountRow() {
		String sql = "select count(*) from shuijian";
		try{
			QueryRunner runner = new QueryRunner(DaoUtils.getSource());
	...
	public List<Cust> getCustByPage(int from, int count) {
		String sql = "select * from shuijian limit ?,?";
		try{
			QueryRunner runner = new QueryRunner(DaoUtils.getSource());
