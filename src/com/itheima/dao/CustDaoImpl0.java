...
public class CustDaoImpl0 {
	public void addCust(Cust cust) {
		String sql = "insert into ningjintai values (null,?,?,?,?,?,?,?,?)";
		try{
	...
	public Cust findUserByProductid(String productid) {
		String sql = "select * from ningjintai where productid = ?";
		try{
			QueryRunner runner = new QueryRunner(DaoUtils.getSource());
	...
	public List<Cust> getAllCust() {
		String sql = "select * from ningjintai";
		try{
			QueryRunner runner = new QueryRunner(DaoUtils.getSource());
	...
	public Cust findUserById(String id) {
		String sql = "select * from ningjintai where id = ?";
		try{
			QueryRunner runner = new QueryRunner(DaoUtils.getSource());
	...
	public void updateCust(Cust cust) {
		String sql = "update ningjintai set xinghao=?,pihao=?,parame1=?,parame1p=?,parame2=?,parame2p=?,time=? where productid=?";
		try{
			QueryRunner runner = new QueryRunner(DaoUtils.getSource());
	...
	public void delCustByID(String id) {
		String sql = "delete from ningjintai where id = ?";
		try{
			QueryRunner runner = new QueryRunner(DaoUtils.getSource());
	...
	public List<Cust> findCustByCond(Cust cust) {
		String sql = "select * from ningjintai where 1=1 ";
	...
	public int getCountRow() {
		String sql = "select count(*) from ningjintai";
		try{
			QueryRunner runner = new QueryRunner(DaoUtils.getSource());
	...
	public List<Cust> getCustByPage(int from, int count) {
		String sql = "select * from ningjintai limit ?,?";
		try{
			QueryRunner runner = new QueryRunner(DaoUtils.getSource());
